...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (existingCard) {
    ...
    if (group) embed.addFields({ name: 'Group', value: `${existingCard.group} → ${group}`, });
    if (rarity) embed.addFields({ name: 'Rarity', value: `${existingCard.rarity} → ${rarity}`, });
    if (era) embed.addFields({ name: 'Era', value: `${existingCard.era || 'None'} → ${era}`, });
    if (droppable !== null) embed.addFields({ name: 'Droppable', value: droppable ? '🏘️ Yes' : '🧚 No', });
  ...
  if (era) {
    embed.addFields({ name: 'Era', value: era, });
  }
